"""Validate conveyor HDF5 — mass-invariant, energy, finite-diff, contact-energy."""
import os, sys
import h5py
import numpy as np
...
def assert_contact_energy(data, attrs, r_thresh=0.6):
    # Relaxed threshold: sustained sliding on belt produces weaker impulse-KE
    # correlation than transient impacts (see ADR-0009 § contact-energy).
...
def main(h5_path=None):
    if h5_path is None:
        ...
        h5_path = os.path.join(repo, "_output", "conveyor", "conveyor.h5")
